...
This library provides a collection of functions that help implement quantum cluster methods.
...
This document also provides insights as to the inner working of the code, and therefore constitutes also an embryonic developer's manual.
...
The previous instructions, however, do not install an optimized executable and
...
* ``-DPRIMME_DIR=[path_to_PRIMME_root_dir]``: Specify the path to the PRIMME root directory for linking qcm_wed library against if not download automatically (default not set).
...
* ``-DCMAKE_C_COMPILER=[gcc/icc/clang/...]``: The C compiler to use to compile qcm and its automatically downloaded dependencies.
* ``-DCMAKE_CXX_COMPILER=[g++/icpc/clang++/...]``: The C++ compiler to use to compile qcm and its automatically downloaded dependencies (must match the C compiler, for example ``gcc`` with ``g++``).
