...
class cluster_model:
    ...
    def __init__(
        ...
        self.bath_irrep = bath_irrep
        self.gbath = None
        self.is_closed = False
        self.operators = []  # list of (op_name, op_type, elem, is_complex)
    ...
    def new_operator(self, op_name, op_type, elem):
        ...
        self.operators.append((op_name, op_type, elem, False))
...
class lattice_model:
    ...
    def __init__(self, name, clus, superlattice, lattice=None, hybrid_file=""):
        ...
        self.has_bath = False
        self.operator_calls = []  # list of (method_name, args, kwargs) for write_definition
        self.target_sectors = None
        ...
        if isinstance(params, str):
            ...
            for p in re.split("[,;\n]", params):
                ...
                if param_name in param_set:
                    raise ParseError(
                        "parameter " + param_name + " has already been assigned!"
                    )
    ...
    def set_params_from_file(self, out_file, n=0, pr=False):
        ...
        if n >= D.shape[0]:
            raise ValueError(
                "The data set has only {:d} solutions (<= {:d})".format(D.shape[0], n)
            )
        for x in par:
            if par[x][1] != None:
                continue
    ...
    def write_definition(self, filename=None, str=False):
        ...
        The generated file contains calls to cluster_model, cluster and lattice_model
        constructors and their operator-adding methods, reproducing the full model definition.

        :param str filename: path of the Python file to write. Required unless str=True.
    ...
    from ._loop import (  # ,flexible_loop
        Hartree_procedure,
        controlled_fade,
        controlled_loop,
        fade,
...
class model_instance:
    ...
    def read_hdf5(self, filename, sys=0, set_parameters=False):
        ...
        if set_parameters:
            ...
            for name, val in params.items():
                ...
                elif name + suffix in valid:
                    self.model.set_parameter(name + suffix, val)
    ...
    def write_impurity_problem(self, s=0, bath_diag=False, file="impurity.tsv"):
        ...
        with open(file, "w") as f:
            f.writelines(
                "n_sites: {:d}\nn_bath: {:d}\nmixing: {:d}\n".format(
                    CM.n_sites, CM.n_bath, self.model.mixing
                )
            )
            if self.is_complex:
    ...
    def Lehmann_Green_function(self, k, orb=1, spin_down=False):
        ...
        if self.model.has_bath:
            raise ValueError(
                "The lattice model contains clusters with bath orbitals. The Lehmann representation of the lattice Green function cannot be obtained."
            )
    ...
    def QP_weight(self, k, eta=0.01, orb=1, spin_down=False):
        ...
        if len(sigma1.shape) == 3:
            Z = (
                sigma1[:, orb - 1, orb - 1].imag - sigma2[:, orb - 1, orb - 1].imag
            ) / (2 * eta) + np.ones(len(k))
        else:
    ...
    def spin_spectral_function(self, freq, k, orb=None):
        ...
        if self.model.mixing != 2 and self.model.mixing != 3:
            raise RuntimeError(
                'The function "spin_spectral_function()" makes sense only if spin-flip terms are present'
            )
    ...
    from ._spectral import (
        ...
        compute_spectral_function_shared,
        gap,
        mdc,
        mdc_anomalous,
        monopole,
...
class hartree:
    ...
    def update(self, I, pr=False):
        ...
        if pr:
            print(
                "delta {:s} = {:1.3g} ( {:1.3g}%)".format(
                    self.Vm, self.diff, 100 * self.diff_rel
                ),
                meta_pr,
            )
    ...
    def __str__(self):
        return (
            ...
            + ", mean-field operator "
            + self.Vm
            + ", coupling = {:f}".format(self.eig)
        )

    # -----------------------------------------------------------------------------------------------
    def print(self):
        print(
            "<{:s}> = {:g}\t{:s} = {:g} (diff = {:g}, {:g}%)".format(
...
def wavevector_path(n=32, shape="triangle"):
    ...
    if isinstance(shape, tuple):
        ...
        shape == "tetragonal2"
    ):  # tetragonal lattice (000)-(100)-(1/201/4)-(1/21/21/4)-(1/21/20)
        k = np.zeros((4 * n + 1, 3))
        for i in range(n):
            k[i, 0] = i / n
            k[i, 1] = 0
...
def orbital_manager(orbitals, from_zero=False, spin_split=False):
    ...
    if orbitals is None:
        ...
        raise ValueError(
            "The list of orbitals does not have the right format (None, int, tuple or list)"
        )
    if from_zero:
...
def is_sequence(obj):
    ...
    """
    if isinstance(obj, list) or isinstance(obj, tuple) or isinstance(obj, np.ndarray):
        return True
    else:
        return False
...
def read_model_instance(filename):
    """
    Reconstructs a model and its solved instance from an HDF5 file written by
    ...
    I = model_instance(model)
    I.read_all_hdf5(filename, set_parameters=True)
    return I
...
def _clenshaw_curtis_nodes_weights(n):
    """
    Nodes and Clenshaw-Curtis (Fejer first-rule) weights for the integration
    of a plain function over the interval [-1,1].

    The nodes are the first-kind Chebyshev nodes x_k = cos((2k-1)*pi/(2n)),
    which are interior to (-1,1) (an open rule, so the semi-infinite tail
    mapping stays finite). Unlike the Gauss-Chebyshev weights, these weights
    carry no 1/sqrt(1-x^2) factor: they integrate a plain function and sum to 2
    (the length of [-1,1]).

    :param int n: number of points
    :returns: (nodes, weights), both ascending in the node coordinate
    """
    k = np.arange(1, n + 1)
    theta = (2 * k - 1) * np.pi / (2 * n)  # descending nodes = cos(theta)
    j = np.arange(1, n // 2 + 1)
    # Fejer first-rule weights (Waldvogel 2006)
    c = np.cos(2.0 * np.outer(theta, j)) / (4.0 * j * j - 1.0)
    weights = (2.0 / n) * (1.0 - 2.0 * c.sum(axis=1))
    nodes = np.cos(theta)
    # return ascending in the node coordinate
    return np.flip(nodes), np.flip(weights)


def chebyshev_frequency_grid(w1, w2, n1, n2, n3):
    """
    returns a frequency grid (along the positive imaginary axis) tailored for integration.
    It is a Clenshaw-Curtis (Chebyshev) grid of n1, n2 and n3 points in each of the intervals [0,w1], [w1,w2] and [w2,infinity]
    ...
    nodes, weights = _clenshaw_curtis_nodes_weights(n1)
    w = np.zeros(n1+n2+n3)
    p = np.zeros(n1+n2+n3)
    w[0:n1] = w1 * 0.5 * (nodes + 1)
    p[0:n1] = weights * w1 * 0.5
    nodes, weights = _clenshaw_curtis_nodes_weights(n2)
    w[n1 : n1+n2] = w1 + 0.5 * (w2 - w1) * (nodes + 1)
    p[n1 : n1+n2] = weights * (w2 - w1) * 0.5
    nodes, weights = _clenshaw_curtis_nodes_weights(n3)
    w[n1+n2 :] = 1.0 / ((1 / w2) * 0.5 * np.flip((nodes + 1)))
    p[n1+n2 :] = (
        np.flip(weights) * (1 / w2) * 0.5 * w[n1+n2 :] * w[n1+n2 :]
    )
