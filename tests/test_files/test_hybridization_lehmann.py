# Test file
# Goal : check model_instance.hybridization_function_lehmann() by printing its
#        Lehmann representation and comparing it against a direct evaluation
#        of hybridization_function_sys().
# --------------------------------------------------------------------------------
import numpy as np
...
# (same model as test_cdmft_trf.py, without running CDMFT)
# ----------------------------------------------------------------
...
rep = I.hybridization_function_lehmann(sys=0)
print("Lehmann representation of the hybridization function:")
print("poles W =\n", rep.W)
print("amplitudes Q =\n", rep.Q)

# --------------------------------------------------------------------------------
# consistency check against the direct frequency-domain evaluation

z = 0.7 + 0.3j
Gamma_lehmann = rep.evaluate(z)
Gamma_ref = I.hybridization_function_sys(z, sys=0)
err = np.max(np.abs(Gamma_lehmann - Gamma_ref))
print("max|diff| =", err)
assert err < 1e-9

print("test_hybridization_lehmann: all checks passed")
